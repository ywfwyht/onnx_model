...
class VitSegNet(nn.Module):
    def __init__(self,
                ...
                dropout=0.,
                emb_dropout=0.,
                is_with_shared_mlp=True, # row False, seg True
    ...
    def forward(self, img):
        ...
        # print(f'1: {x.shape}') # 1: torch.Size([6, 324, 512])
        
        x += self.pos_embedding[:, :n]
        x = self.dropout(x)
        
        x = self.transformer(x)
        # print(f'2: {x.shape}') # 2: torch.Size([6, 324, 512])
        # return x

        x = self.rearrange(x)
        # return x
        # print(f'3: {x.shape}') # 3: torch.Size([6, 8, 144, 144])
        
        # x = torch.randint(0, 2, (1, 8, 144, 144), dtype=torch.float32).cuda()

        if self.is_with_shared_mlp:
            x_mlp = self.shared_mlp(x)
        # print(f'4: {x.shape}') # 4: torch.Size([6, 8, 144, 144])
        # img: torch.Size([6, 64, 144, 144]) 
        # x: torch.Size([6, 8, 144, 144])
...
if __name__ == '__main__':
    ...
    preds = v(img)
    # is_with_shared_mlp=False, torch.Size([1, 8, 144, 144])
    # is_with_shared_mlp=True, torch.Size([1, 1024, 144, 144])
