...
class GridSeg(nn.Module):
    ...
    def forward(self, x):
        # x: torch.Size([6, 1024, 144, 144])
        conf_output = self.act_sigmoid(self.conf_predictor(x)) # torch.Size([6, 1, 144, 144])
        class_output = self.class_predictor(x) # torch.Size([6, 7, 144, 144])
    ...
    def get_lane_map_numpy_with_label(self, output, label, is_flip=True, is_img=False):
        ...
        *       'rgb_img_conf_cls_idx': (144, 144, 3)
        ...
        list_point_idx = [] # for 3d pointcloud space

        batch_size = len(output['conf'])
        for batch_idx in range(batch_size):
            # cls_label = data['label'][batch_idx].cpu().numpy()
            cls_label = label[batch_idx].cpu().numpy()
            ...
            print('exist: ', conf_pred_raw, np.unique(conf_pred))
            ...
            print('cls1: ', np.unique(cls_idx))
            cls_idx[np.where(cls_idx==6)] = 255
            print('cls2: ', np.unique(cls_idx))
    ...
    def get_lane_map_numpy_with_predict(self, output, data, is_flip=True, is_img=False):
        '''
        * in : output feature map
        * out: lane map with class or confidence
        *       per batch
        ...
        *       'rgb_img_conf_cls_idx': (144, 144, 3)
        ...
        for batch_idx in range(batch_size):
            # cls_label = data['label'][batch_idx].cpu().numpy()
            # cls_label = data[batch_idx].cpu().numpy()
            # conf_label = np.where(cls_label == 255, 0, 1)

            conf_pred_raw = output['conf'][batch_idx].cpu().numpy()
            # print('ccccccccccccc ',conf_pred_raw, np.unique(conf_pred_raw))
            if is_flip:
                conf_pred_raw = np.flip(np.flip(conf_pred_raw, 0),1)
            # print('ccccccccccccc ',conf_pred_raw, np.unique(conf_pred_raw))
            conf_pred = np.where(conf_pred_raw > self.cfg.conf_thr, 1, 0) # > 为1, <为0
            # print(np.unique(conf_pred))
            cls_pred_raw = torch.nn.functional.softmax(output['cls'][batch_idx], dim=0)
            cls_pred_raw = cls_pred_raw.cpu().numpy()
            if is_flip:
                cls_pred_raw = np.flip(np.flip(cls_pred_raw, 1),2)
            ...
            xy_idx = data['img_idx']
            cls_idx = np.flip(np.flip(cls_idx, axis=0), axis=1)
            point_cls = cls_idx[xy_idx[:,0] // 8, xy_idx[:,1] // 8]
            point_cls = point_cls.reshape(-1,1)
            point_cls = np.concatenate([data['points'], point_cls], axis=1)
            list_point_idx.append(point_cls)
