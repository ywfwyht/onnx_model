...
class Projector(nn.Module):
    ...
    def forward(self, sample):
        # proj = sample['proj']  # torch.Size([1, 3, 1152, 1152])
        proj = sample  # torch.Size([1, 3, 1152, 1152])
        # print("projector input: ",proj.shape)
        out = self.resnet(proj) # torch.Size([1, 64, 144, 144])
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = conv1x1(inplanes, width) # 加速收敛，提高泛化能力，防止过拟合，使每一层输入保持相同分布
        self.bn1 = norm_layer(width) # 使小于0的值为0，大于等于0的值保持不变
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            identity = self.downsample(x) # 残差residual
